alter table public.patient_payments
  add column if not exists reference_amount numeric(10,2) not null default 0,
  add column if not exists discount_amount numeric(10,2) not null default 0,
  add column if not exists surcharge_amount numeric(10,2) not null default 0;

alter table public.professional_payouts
  add column if not exists reference_amount numeric(10,2) not null default 0,
  add column if not exists discount_amount numeric(10,2) not null default 0,
  add column if not exists surcharge_amount numeric(10,2) not null default 0;

alter table public.appointment_financials
  add column if not exists professional_payout_status text not null default 'Pendente';

alter table public.appointment_financials
  drop constraint if exists appointment_financials_professional_payout_status_check;
alter table public.appointment_financials
  add constraint appointment_financials_professional_payout_status_check
  check (professional_payout_status in ('Pendente', 'Repassado'));

alter table public.patient_payment_items
  drop constraint if exists patient_payment_items_amount_check;
alter table public.patient_payment_items
  add constraint patient_payment_items_amount_check check (amount >= 0);

alter table public.professional_payout_items
  drop constraint if exists professional_payout_items_amount_check;
alter table public.professional_payout_items
  add constraint professional_payout_items_amount_check check (amount >= 0);

drop function if exists public.register_patient_payment(uuid, date, numeric, text, text, uuid[]);
drop function if exists public.register_professional_payout(uuid, date, numeric, text, text, uuid[]);
...
begin
  ...
  foreach v_appointment_id in array p_appointment_ids loop
    select round(coalesce(af.consultation_fee, 0), 2)
      into v_fee
  ...
  foreach v_appointment_id in array p_appointment_ids loop
    ...
    values (v_payment_id, v_appointment_id, v_fee);
...
begin
  ...
  foreach v_appointment_id in array p_appointment_ids loop
    select round(coalesce(af.consultation_fee, 0) * 0.70, 2)
      into v_due
    from public.appointments a
    left join public.appointment_financials af on af.appointment_id = a.id
